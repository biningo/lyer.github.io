...
大部分程序都是采用缓冲IO来访问磁盘等数据，这样可以借助操作系统减少访问磁盘的次数。但是有些程序比如**数据库MySql**等则不会使用缓存IO，而是自己设计一套更加优秀的缓存机制，这样可以减少内核空间到用户空间拷贝数据的消耗。并且数据库都是有一套自己数据保存磁盘上的规则，比操作系统更加了解如何快速的访问数据
